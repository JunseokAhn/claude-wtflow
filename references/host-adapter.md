...
- **자체호스팅은 주소만으로 판별하지 못한다.** `gitlab.example.com` 처럼 이름에 단서가 있으면 그걸 쓴다
...
- ⚠️ **API 로 내려가도 본문 쓰기는 그대로 계약 대상이다** — `body-rewrite.md`
...
⚠️ **본문 쓰기는 어느 호스트든 계약 대상이다** — 정당한 경로·환경변수·파일 경유는
`body-rewrite.md` 가 갖는다. **여기 사본을 두지 않는다.**

- 표에 없는 하는 일(마일스톤 등)은 `## CLI 로 안 되는 것은 API 로 내려간다` 로 간다

## 마일스톤 — 조직 레벨이 있는 호스트와 없는 호스트
...
**조직 레벨이 없는 호스트에서는 저장소 마일스톤으로 내리고, 여러 저장소를 묶는 일은 마일스톤 note 가 맡는다.**
...
- ⚠️ **없는 조직 레벨을 흉내내지 않는다** — 조직 레벨 마일스톤이 없는 호스트에서 그걸 만들려고
  API 를 뒤지지 않는다. 없으면 없는 것이다

**note 경로에 쓰는 식별자도 다르다** — `~/.claude/notes/<group>/_milestone/<식별자>-<slug>.md`:
