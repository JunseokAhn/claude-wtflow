...
#
# 유일한 예외는 wtflow:commit 의 작업 항목 체크박스 동기화다. 그건 본문을 다시 쓰는 게 아니라
# `- [ ]` 한 줄을 켜는 것이라 계약을 탈 이유가 없다. 그 호출은 명령 앞에 센티넬을 붙여
# 스스로를 밝힌다 — 훅은 명령 문자열만 보므로, 호출자가 밝히는 것 말고 구별할 방법이 없다.
#
# 판정 못 하면 통과시킨다(fail-open). 훅이 못 읽는 payload 때문에 작업이 서는 것이,
# 막지 못한 본문 수정 하나보다 나쁘다.
set -u

SENTINEL='WTFLOW_CHECKBOX_SYNC=1'
...
# 예외 — wtflow:commit 의 체크박스 동기화
case "$cmd" in *"$SENTINEL"*) exit 0 ;; esac
