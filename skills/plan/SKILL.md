...
9. **완료 조건 — 계획 단계에서 1회** (§10 수용 기준). 실행은 계약 8 의 표를 낸 직후이고,
   **§9 커밋 경계와 같은 `AskUserQuestion` 호출에 담는다** — 호출을 나누면 계획 한 번에 멈춤이 둘이 된다.
   ...
   | | |
   |---|---|
   | 이슈 작업 | 고른 조건을 이슈 본문 `## 완료 조건` 에 쓴다 — **섹션이 없으면 `작업 항목` 뒤에 만든다** |
   ...
   - 건너뛰는 자리(동작이 안 바뀌는 이슈 등)에 걸리면 묻지 않고 **건너뛴 사실을 한 줄로 적는다** —
     그런 이슈에는 `## 완료 조건` 섹션을 만들지 않는다
   - **재플랜에서는 이미 있는 조건을 다시 본다**(`plan-replan.md`) — 새로 고르지 않는다
...
10. **다음 단계 안내 — plan 의 마지막 출력** — 계약 9 까지 끝나면 이 블록으로 끝낸다.
    ...
    ```
    다음 단계 — 이어서 할 일:
      /wtflow:auto                  남은 작업을 끝까지
      /wtflow:auto --no-quiz        코드 동작을 묻는 질문만 빼고 끝까지
      /wtflow:auto --no-ask         의도 확인을 묻는 질문도 없이 끝까지
      /wtflow:auto --step 2         두 번째 작업까지 하고 멈춤
    ```

    - **비개발자도 읽히는 말로 쓴다** — 설명 칸에 `❓`·절 이름(코드 읽기·동작 확인)·`Step` 을 쓰지 않는다
    - **한 줄에 명령 하나.** 설명을 다음 줄로 내리지 않는다 — preview 폭(46자)에 맞춘 줄바꿈을
      여기 옮겨 오면 터미널에서 명령과 설명이 갈라져 보인다
    - `--step` 줄의 번호는 **남은 작업 중 두 번째**다 — 재플랜이면 그만큼 올려 적고(`--step 4` →
      「네 번째 작업까지 하고 멈춤」), 남은 작업이 하나뿐이면 이 줄을 뺀다
