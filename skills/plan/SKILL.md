...
8. **plan 출력** — 작업 정의 + 계약 6 의 계획 문서 + **코드 분석 결과**로 작업 단계 추정
   ...
   **어느 K 가 사전문답 대상인지도 이때 표시**한다(`### 사전문답 대상 표시`) — 착수 후에 정하면
   이미 구현이 시작돼 `learning-protocol.md` 의 §1 예측이 성립하지 않는다
...
3. **Step = 작업 항목** — 한 작업 항목 = 한 K = 한 mirror 분기, 커밋은 그 안에 누적
   - **작업 항목이 이미 있어도 코드 탐색 결과로 다시 뽑는다**(`learning-protocol.md` §9 커밋 경계).
   ...
   - **없으면 여기서 만든다**(adhoc 의 첫 plan 이 항상 여기다) — 확정한 범위 + 코드 분석 결과를
     ...
     (`learning-protocol.md` §9 커밋 경계 — 확인이 아니라 선택이고, `Na` 문항과 같은 호출에 담는다.
     분해안이 K 경계로 보아 하나면 묻지 않는다 — `## 질문을 내기 전 검사` ①)
...
- 대상은 두 갈래다 — **동작 판단이 든 K**(§1 예측)와 **물어야 할 설계 선택이 있는 K**(§4 설계 선택).
  후자는 `learning-protocol.md` §4 설계 선택 `### 묻는 기준` 의 두 잣대(되돌리기 비용·파급 범위)를 **둘 다**
  통과할 때만이고, **K 당 1건 상한**이다. 통과하는 게 없으면 **그 K 엔 `❓` 를 안 붙인다** —
  K 마다 하나씩 짜내면 질문이 구현 세부로 내려가 마찰만 남는다.
  삭제·정리·보일러플레이트에는 안 붙인다
- **붙이기 전에 `learning-protocol.md` `## 질문을 내기 전 검사` 를 거친다** — 안들의 산출물이 같거나
  사실이 이미 답을 정하면 `❓` 를 붙이지 않는다. 붙인 K 라도 시작 직전에 같은 검사에 걸리면 묻지
  않고, **건너뛴 사실과 걸린 검사를 한 줄로 적는다**
- **질문은 산문이 아니라 `AskUserQuestion` 으로 낸다** — §1 예측·§4 설계 선택·§5 변경 시나리오 전부(`learning-protocol.md`
  `## 사전문답은 전부 AskUserQuestion 으로 낸다`). 한 K 의 문항은 **한 번의 호출에 최대 3문항**
  으로 모은다. 산문으로 늘어놓으면 답 없이 넘어가 트리거가 안 걸린다
- **`(추천)` 은 §4 설계 선택에만 붙인다.** §4 설계 선택 보기에는 **판단 기준 한 줄**과 첫 보기 `(추천)` 을 반드시
  넣는다(§4 설계 선택 `### 형식`) — 트레이드오프만 나열하면 고를 수가 없다. 반대로 **§1 예측·§5 변경 시나리오
  보기에는 `(추천)` 을 붙이지 않는다** — 맞히는 자리라 추천이 곧 정답 누설이다
- **`file:line` 만 던지지 않는다.** 답에 필요한 코드를 인용하고, 설계 선택은 좌우 비교로 띄운다.
  **두는 자리는 `learning-protocol.md` §1 예측 `### 어디에 두나` 가 정한다** — preview 에 들어가면
  preview, 넘치면 질문 앞 본문(§1 예측 `### 질문은 자족적이어야 한다` · §4 설계 선택 `### 형식`)
...
- **물어서 고른 분할은 뭉개지 않는다**(`learning-protocol.md` §9 커밋 경계). 구현 중 더 잘게 쪼개는 건
