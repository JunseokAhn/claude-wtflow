...
| Step | 설명 | 관련 파일/위치 |
|------|------|----------------|
| 1 | <한 줄 요약> | `path/to/file.kt:42` |
| 2 ❓ | ... | `path/to/other.kt:88` 외 2 |

각 K 의 커밋은 `/wtflow:commit "<메시지>" -K <Step>` 이다. **호출을 행마다 적지 않는다** —
Step 번호에서 그대로 나오고, 커밋 메시지는 착수 전 plan 이 아니라 커밋 시점에 짓는다.
...
|---|---|
| `설명` 열 한 칸 | 1줄 |
| `관련 파일/위치` 열 한 칸 | 3개 — 넘으면 대표 하나 + `외 N` |
| 표 아래 `❓` 예고 | K 당 2줄 |

Step 행 수는 이 셈에서 뺀다 — 개수가 작업 크기에서 나온다.
탐색한 파일을 다 싣고 싶으면 `관련 파일/위치` 가 아니라 그 K 를 시작할 때 낸다.
...
- **붙이기 전에 `learning-protocol.md` 의 `## 질문을 내기 전 검사` 를 거친다.** 붙인 K 라도 시작
  직전에 같은 검사에 걸리면 묻지 않고, **건너뛴 사실과 걸린 검사를 한 줄로 적는다**
...
- **Step 번호 = K 번호 = 작업 항목 번호**, mirror 는 `<accumulator>-00N`. 커밋 누적·멈추는 자리는
  `worktree-discipline.md` 가, 커밋 절차는 `/wtflow:commit` 계약이 갖는다
- **plan 의 K 매핑이 곧 체크박스 순서다** — K 를 항목 번호에 정확히 고정할 것. 어긋나면 `--done` 이
  엉뚱한 줄을 켠다
