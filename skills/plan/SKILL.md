...
⚠️ **plan 은 어떤 파일도 쓰지 않는다.** 작업 항목을 저장하지 않고, 계약 6 에서 읽은 note 도
고치지 않는다. 이 스킬이 `EnterWorktree` 로 같은 세션을 워크트리에 밀어넣으므로 plan 출력이
그대로 살아 있고, 세션이 끊겨도 K 번호와 커밋은 mirror 분기에 남는다.
...
6. **계획 문서 로드 (plan 출력 전 필수)** — 착수 전에 정해둔 계약이 있는지 본다. **읽기만 한다.**
   ...
   ⚠️ **이 스킬은 note 를 쓰지 않는다 — 만들지도, 고치지도, 갱신하지도 않는다.** 이슈 note 는
   `/wtflow:issue` 가 1회 작성한 뒤 아무도 손대지 않는 스냅샷이다. plan 이 갱신하기 시작하면
   진행 중 계속 손봐야 하고, 그게 원래 이 파일을 못 쓰게 만들었던 비용이다.

   ⚠️ **note 가 낡았으면 이슈 본문·현재 코드가 이긴다.** `created` 날짜 이후에 계약이 바뀌었을 수
   있다. note 와 이슈 작업 항목이 어긋나면 **이슈가 진실원**이고, note 는 참고로만 쓴다.
   고칠 필요가 보이면 **사용자에게 알리되 스스로 고치지 않는다.**

   ⚠️ **마일스톤 목록을 훑지 않는다** — `_milestone/` 디렉토리를 글롭해 "아마 이거겠지" 로 고르는 것
   금지. 이 이슈의 계약인지는 **배정**만이 답할 수 있고, 무관한 마일스톤을 계약으로 오인하면
   plan 전체가 틀어진다.

   **왜 마일스톤은 사본도 심링크도 두지 않나** — 배정은 계약 1 의 이슈 조회에 딸려 오므로 경로를
   아는 데 드는 추가 비용이 0이다. 워크트리에 심링크를 굳혀두면 재배정됐을 때 옛 파일을 가리킨
...
- **이 스킬은 note 를 만들지도 고치지도 않는다.** 이슈 note 의 형식과 생성은 `/wtflow:issue`,
  마일스톤 note 는 `/wtflow:milestone` 몫이고, 이 스킬은 계약 6 에서 **읽기만** 한다
