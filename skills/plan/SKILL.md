...
⚠️ **plan 은 작업 항목을 파일로 저장하지 않는다.** 이 스킬이 `EnterWorktree` 로 같은 세션을
워크트리에 밀어넣으므로 plan 출력이 그대로 살아 있고, 세션이 끊겨도 K 번호와 커밋은 mirror 분기에
남는다. 파일로 남는 note 는 여러 이슈가 공유하는 마일스톤 계약 하나뿐이다(계약 6).
...
8. **plan 출력** — 작업 정의 + 계약 6 의 마일스톤 계약 + **코드 분석 결과**로 작업 단계 추정
...
0. **마일스톤 계약 로드** — 계약 6 과 동일. 심링크가 이미 있으니 Read 한 번으로 끝난다.
   **진행 상태를 여기서 읽지 않는다** — 상태는 1번에서만 온다
1. **진행 상태 수집**
...
- **이 스킬은 note 를 만들지도 고치지도 않는다.** 마일스톤 note 의 형식과 생성은 `/wtflow:milestone`
  몫이고, 이 스킬은 계약 6 에서 심링크를 걸어 **읽기만** 한다
