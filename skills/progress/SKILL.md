---
name: progress
description: 워크트리 작업(이슈/adhoc)의 Step 진행 현황을 표로 출력(읽기 전용). 이슈 작업은 본문 체크리스트 + mirror 분기, 이슈 없는 작업은 mirror 분기만으로 판정. 사용자 호출 + wtflow:commit/auto가 매 커밋 후 자율 호출.
...
**활동 오버레이(Step 귀속 소스 아님).** **미귀속 작업**(dirty **또는** 어떤 mirror 도 도달 못 하는 raw 커밋)이 있으면 활동 신호로 보고 **활성 Step(절차 7)를 🟡 진행 중** 으로 올린다. 단정 불가라 활성 Step 한 곳에만 적용하고 파일 수·미분류 커밋은 표 아래로 노출. "커밋했는데 대기로 보이는" 착시를 막는 best-effort 이고, 정확한 귀속은 wtflow:commit 담당.
...
"Step i 에 커밋이 잡혔다" = mirror `-<i>` 분기가 있다. 상태는 **완료/진행 중/대기 셋뿐** — "진행 중"은 귀속 커밋이든 미귀속 작업이든 하나로 합친다.
...
|------|------|
| 체크리스트 `- [x]` | ✅ 완료 |
| `- [ ]` + (커밋 파악됨 **또는** 활성 Step & 미귀속 작업 있음) | 🟡 진행 중 |
| `- [ ]` + 그 외 | ⬜ 대기 |
...
| 조건 | 표기 |
|------|------|
| mirror 있음 + 활성 Step 아님 | ✅ 완료 |
| mirror 있음 + 활성 Step | 🟡 진행 중 |

⚠️ 이슈 없는 모드에 **⬜ 대기는 없다** — 아직 커밋 없는 Step 은 mirror 도 없어 존재 자체를 알 수 없다.
"완료"는 "이 Step 에서 더 커밋하지 않았다"는 뜻이지 항목이 닫혔다는 선언이 아니다(선언할 곳이 없다).

- **완료(✅)는 dirty 여도 그대로** — 닫힌 Step 은 활성 Step 산정에서 제외
- **현재 Step 표시**: 활성 Step 행 앞에 `▸`
- **계획 외 Step**(이슈 모드만): 항목 수보다 큰 Step(mirror)이 있으면 표 아래 `(계획 외 Step{n}: <tip>)` 한 줄. 추측해서 항목에 끼워 넣지 않는다
...
커밋된 Step <n>개  ·  현재 Step<현재>
작업 중: <n>개 파일 변경        ← dirty 일 때만
```

- 최신 커밋 열 = 그 Step mirror tip 의 `%h %s`. 미귀속 작업으로만 진행 중이면 `— (작업 중)`(dirty) 또는 `— (미분류 <n>)`
...
- 이슈 없는 모드 + mirror 0개 → "아직 커밋된 Step 이 없다 — `/wtflow:commit` 으로 첫 Step 을 커밋하면 표가 나온다"
- `작업 항목` 체크리스트 없음(이슈 모드) → "체크리스트형 아님 — 진행 현황 대상 없음"
- 섹션/항목 순서 모호 → 추측 매핑 금지, 모호하다고만 보고

## 비고

- **wtflow:commit** 이 커밋·체크박스 동기화·요약 직후 `--quiet` 로 1회 호출한다(commit 계약 8). **wtflow:auto** 는 커밋을 commit 에 위임하므로 Step마다 자동으로 이 표가 따라온다. 이 스킬이 종료돼도 커밋 흐름은 막지 않는다 — 진행 가시성용 부가 출력일 뿐
...
- **이슈 없는 모드는 표가 자라기만 한다** — Step 이 커밋돼야 행이 생기므로 계획 대비 진척률을 보여주지 못한다. 그게 필요하면 `/wtflow:issue` 로 승격해 체크리스트를 갖는다(`plan-adhoc.md` 의 `### 이슈로 승격`)
