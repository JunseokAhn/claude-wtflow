...
5. **반영 → push → 생성 순서를 지킨다** — 뒤집지 않는다:
   ...
   - 머지 규칙(메시지 형태·기준 브랜치 판정·충돌 해소·검증)은 **전부 wtflow:merge 의 계약**이다.
     여기에 다시 적지 않는다 — 두 벌이 되면 한쪽만 고쳐져 갈린다
...
- 형식·분량은 briefing 의 `### MR description 압축본` 이 정한다. **여기 사본을 두지 않는다**
...
| 항목 | 형태 |
|---|---|
| 헤더 | `6개 고정 — 유지`, 어긋나면 어긋난 이름을 전부 |
| 줄 수 | `31줄 → 38줄 (+7)` — 상한 40줄 대비 |
| `무엇을 바꿨나` | `4줄 / 커밋 5개` — 어긋나면 그대로 표시 |
| 제목 | `변경 없음` 또는 `이전 → 이후` |

- 헤더가 6개에서 벗어났거나 본문이 40줄을 넘으면 **묻지 말고 다시 쓴다** — 물어서 통과시킬
  대상이 아니다
