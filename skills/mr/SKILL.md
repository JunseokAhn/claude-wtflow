...
1. **accumulator 를 origin 에 push 한다 — 규율의 예외다.** 워크트리 브랜치는 로컬 전용이고
   ...
   **스쿼시 브랜치(`<accumulator>-squash`)가 있으면 그것이 소스다.** 접어 둔 것이 곧 리뷰
   단위라, 접어놓고 원본을 올리면 접은 의미가 없다. 있는지는 이름으로 본다:
   ```
   git branch --list '<accumulator>-squash' --format='%(refname:short)'
   ```
   - **mirror 를 배제하는 근거가 여기엔 안 걸린다** — 스쿼시 브랜치는 조각이 아니라
     base..tip 전체를 접은 것이라 통짜다(`wtflow:squash` 계약 3·7)
   - ⚠️ **낡았는지 재고 경고한다.** 스쿼시 브랜치는 접을 때만 갱신되므로(`wtflow:squash` 계약 5)
     접은 뒤 accumulator 에 쌓인 커밋은 안 들어 있다. **트리 해시로 잰다:**
     ```
     git rev-parse '<accumulator>^{tree}' '<accumulator>-squash^{tree}'
     ```
     두 값이 다르면 낡은 것이다. **다시 접을지 묻고, 안 접으면 그 사실을 요약에 적는다**
   ...
   - `-a` 로 accumulator 를 명시해도 이 규칙은 그대로다. 원본을 굳이 올리려면 스쿼시 브랜치를
     지우고 부른다
...
9. **본문은 브리핑에서 얻는다 — `--mr --no-fence` 로 부른다.** 범위는 accumulator 의 base..HEAD:
   ...
   ```
   - ⚠️ **소스가 스쿼시 브랜치여도 범위는 accumulator 다.** 접힌 쪽은 커밋 메시지가 구간
     하나로 합쳐져 있어 무엇을 왜 했는지가 사라진다 — 올리는 것과 읽고 쓰는 것을 나눈다
