...
6. **접힌 커밋의 메시지는 그 구간의 커밋들에서 만든다** — 지어내지 않는다.
   ...
   - 여럿이면 **구간의 비-churn 커밋 전부의 제목·본문을 읽고 커밋 컨벤션대로 새로 쓴다**
     ...
     - **`요약`** : 원본 커밋들의 제목·본문에서 읽히는 **왜** 를 모아 2줄 안으로 줄인다.
       본문에 `요약` 란이 없어도 재료는 있다 — `fix` 제목이 고친 문제가 곧 그 커밋의 이유다
       - **금지는 추측 하나다** — 원본 제목·본문 어디에도 근거가 없는 이유를 지어 넣지 않는다
       - 란째로 빼는 것은 **읽어 낼 재료가 정말 없을 때만**이다. 쓰기 번거롭다고 빼지 않는다
...
8. **미리보기 → 확인 → 생성 순서를 지킨다.** `--dry-run` 이 아니어도 **항상 미리보기를 먼저 낸다**:
   ...
   - **접힌 구간은 새로 쓴 메시지 전문을 목록 아래에 붙인다**(계약 6) — 제목 한 줄만으로는
     `요약` 이 원본에 근거했는지 볼 수 없다. `(그대로)` 구간은 원본 메시지라 붙이지 않는다
     ```
     ── -002 (7커밋 접힘) ──
     feat(squash): 커밋 스쿼시 스킬 신설
     ...
     Co-Authored-By: …
     ```
   - 확인은 `AskUserQuestion` — 산문으로 "이대로 만들까요?" 나열 금지. **메시지를 고치라는 답이면
     그 구간 메시지만 다시 써서 미리보기를 다시 낸다** — 구간 경계·트리는 그대로다
     - 메시지가 바뀌면 그 구간부터 뒤 커밋의 해시가 전부 바뀐다 → 계약 3 의 `commit-tree` 를
       그 구간부터 다시 잇고 계약 7 의 트리 대조도 다시 돈다. 먼저 만든 오브젝트는 GC 가 걷어간다
