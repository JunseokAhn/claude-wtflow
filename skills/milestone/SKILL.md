...
**시작 전에 `${CLAUDE_PLUGIN_ROOT}/references/worktree-discipline.md`(브랜치 이름 규칙·Step 모델·note 종류)와 `${CLAUDE_PLUGIN_ROOT}/references/host-adapter.md`(이슈 호스트 판별·CLI 대응), `${CLAUDE_PLUGIN_ROOT}/references/note-format.md`(마일스톤 note 규격·상태 금지)를 읽는다.**
...
**두 note 의 갱신 성격이 왜 반대인지는 `worktree-discipline.md` 의
`## 상세는 이슈가 아니라 note 에` 가 갖는다** — 여기 사본을 두지 않는다.

- **단일 실체가 핵심이다.** 이 문서의 수정이 **다른 브랜치·다른 세션에서 즉시 읽혀야** 한다.
  브랜치에 커밋하면 사본이 어긋나 이걸 못 지킨다
...
4. **문서 경로 확정** — `~/.claude/notes/<group>/_milestone/<식별자>-<slug>.md`
   ...
   **본문 규격은 `note-format.md` 의 `## 마일스톤 note 형식`, 무엇을 안 적는지는 같은 문서의
   `## 계약·설계 저장소이지 상태 저장소가 아니다`** — 여기 사본을 두지 않는다
