...
9. **영향 범위 문답 (동작이 바뀌는 커밋만)** — 번호는 뒤지만 **실행은 계약 4 mirror 전진 직후, 계약 6 체크박스 동기화보다 앞이다.** 커밋이 있어야 사용자가 `git show` 로 코드를 놓고 답할 수 있고, 답을 받기 전에 요약이 나가면 문답이 산문에 묻힌다. 절차는 `references/learning-protocol.md` §7 영향 범위, 형식은 그 섹션의 `### 형식`
   ...
   | 단계 | 무엇 |
   |---|---|
   | 판정 | 방금 낸 커밋의 diff 가 **동작을 바꾸나** — 반환형·시그니처·부수효과·조건 분기. 삭제·import 정리·포맷·이름변경·문서만이면 여기서 끝 |
   | 수집 | 바뀐 함수·export 의 **호출부**를 저장소에서 찾는다. 테스트·스크립트·훅도 호출부다 |
   | 질문 | `AskUserQuestion` **multiSelect** — 호출부 목록만 보이고 **어느 것이 깨지는지 묻는다.** 본문에 **commit hash** 를 적어 `git show <hash>` 로 열어 보게 한다. 영향 여부를 미리 채우지 않는다. `(추천)` 금지 |
   | 공개 | 답을 받은 **뒤에** 내 답을 낸다. 갈리면 근거로 다투고, 사실 문제면 실행해서 결론낸다 |
   | 기록 | **커밋 메시지에 적지 않는다** — 커밋 메시지는 변경을 읽는 자리다. 문답은 대화에서 끝나고, 깨진 호출부는 고치는 커밋이 자기 메시지로 설명한다 |

   - **호출부 0곳이면 묻지 않는다.** 0곳이라는 사실만 요약(계약 7)에 한 줄로 적는다
   - **16곳을 넘으면 묻지 않는다** — 보기로 낼 수 없다. 표로 내고 확인만 받되 **묻지 못했다는 사실을 요약(계약 7)에 적는다**
   - **`--no-ask` 로 끌 수 없다.** §1 예측·§4 설계 선택·§5 변경 시나리오와 달리 이건 학습 장치가 아니라 회귀를 막는 자리다
   - 깨지는 호출부가 나와도 **이 커밋은 그대로 둔다.** 그 수정은 **같은 K 의 다음 태스크**다 (§7 영향 범위 `### 깨지는 호출부가 나오면`) — 새 K 로 빼지 않는다. 무엇을 고쳤는지는 그 수정 커밋의 메시지가 설명한다
