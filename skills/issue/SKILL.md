---
name: issue
description: GitLab 어댑터(glab 필요) — 이슈 생성·본문 재작성 + 이슈 note 작성 (브랜치·워크트리는 wtflow:plan 몫). "이슈 만들어줘/등록/GitLab에 올려줘" 와 "이슈 본문 다시 써줘/갱신해줘/반영해줘" 의도에 자율 호출. 이슈 본문을 바꾸는 유일한 경로 — glab issue update 직접 호출 금지. prefix·라벨3종 자동, 이슈 본문은 자족적으로 유지하고 상세는 note 로 분리.
...
4. **본문 작성** — 저장소 이슈 템플릿 우선 사용 (아래 `## 본문 템플릿` 참고). 템플릿 없으면 한 줄 요약
   ...
   - **서술 절은 목록이 기본** — `배경`·`현상`·`목적`·`요약`·`상세 설명`처럼 사실을 늘어놓는 절은 문단 대신 목록으로 쓴다. **한 줄에 한 사실.** 두 문장으로 끝나는 이슈는 문단도 된다. 다만 **세 문장을 넘기는 순간 목록으로 바꾼다** — 문단이 길어지면 훑어서 요점을 집을 수 없다
     ...
     - 같은 취지의 규칙이 이미 `## 이슈 note 형식`(계약 요약은 "산문 말고 표로")과 `/wtflow:milestone` 에 있다. 이 줄은 그걸 **이슈 본문까지 넓힌 것**이고, 어휘를 새로 만들지 않는다
...
| 7·8 번호·브랜치 이름 | **적용 안 함** — 이미 정해져 있다 |
| 9 note | 계약이 실제로 바뀌었을 때만 갱신. 본문 손질뿐이면 손대지 않는다 |
...
- "repo는 X" → `-R X`
- "본문 자세히" → 본문에 맥락 추가 (단, 계약 4 의 서술 절 규칙은 유지 — 상세는 계획서로)
