...
4. **본문 작성** — 저장소 이슈 템플릿 우선, 없으면 플러그인 내장 템플릿 (아래 `## 본문 템플릿` 참고). **어느 쪽이든 `## 작업 항목` 체크리스트는 반드시 남는다**
...
1. **템플릿 탐색 — 저장소 두 경로를 다 본다** (Glob). 한쪽만 보면 다른 호스트의 레포에서
   템플릿이 **있는데도** 없다고 판단해 절을 지어낸다(이게 가장 흔한 실패다):
   - `.gitlab/issue_templates/*.md`
   - `.github/ISSUE_TEMPLATE/*.md`

   둘 다 있으면 remote 호스트에 맞는 쪽(`github.com` → `.github/`), 호스트를 못 가리면 `.github/`

   **저장소에 둘 다 없으면 플러그인 내장 템플릿을 쓴다** — `${CLAUDE_PLUGIN_ROOT}/skills/issue/templates/*.md`.
...
4. **폴백**:
   - 저장소 두 디렉토리 모두 없음 → **내장 템플릿**(1의 마지막). 채우기 절차는 3 그대로다 —
     경로만 다르지 규칙이 갈리지 않는다
   - 매핑된 파일명 없음 (예: `bug.md` 가 `defect.md` 로 명명) → Glob 결과에서 prefix·종류와 유사한 파일명 추정 (`bug|defect|fix` 등). 모호하면 사용자에게 묻기
   - 저장소 템플릿에 매핑할 파일이 아예 없음 (예: `chore:` 인데 `infra.md` 가 없다) →
     **그 종류만 내장 템플릿으로 메운다.** 저장소에 있는 다른 파일을 억지로 끌어다 쓰지 않는다
