...
3. **제목 생성**: `<prefix> <한국어 한 줄 요약>` — scope 를 붙여 `<prefix>(<scope>): …` 로 써도 된다
   ...
   - **두 산출물이면 접속사 말고 `+`** — ✅ `feat: 자연어 KG 이미지 검색 API + KG 화면 연동` / ❌ `feat: 검색 API 를 만들고 화면에 연동한다`
   - **보충은 괄호 하나까지** — `fix: 동시 업로드 시 fileName 중복 생성 방지 (확정 데이터 유니크 인덱스)`
   - **식별자·영어는 원문 유지** — `MEMBER_CREATE_COUNT` · `@ConditionalOnBean` · `gzip` · `work_session`. 억지로 한글화하지 않는다
   - 예: `fix: 사용자 로그인 시 토큰 만료 에러` · `refactor(auth): 리프레시 세션을 불투명 토큰과 슬라이딩 만료로 이관`
