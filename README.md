...
브랜치를 만드는 것이 `/wtflow:commit` 절차 안에 있어서, 그 스킬을 건너뛰고 커밋하면 브랜치가
뒤처집니다. 훅이 그 경로를 막고 `/wtflow:commit` 으로 안내합니다.

| | 막히나 |
|---|---|
| Claude 가 작업 브랜치에서 직접 실행하는 `git commit` | ✅ 거부 — `/wtflow:commit` 으로 안내 |
| `/wtflow:commit` 이 하는 커밋 | ❌ 그대로 진행 |
| 터미널·소스트리 등 **사람이 직접 하는 커밋** | ❌ 영향 없음 — 훅은 Claude 의 도구 호출에만 걸립니다 |
| 충돌 해소 중인 머지 커밋 · 워크트리 밖 커밋 | ❌ 그대로 진행 |
