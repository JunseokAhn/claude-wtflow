...
K 당 5~15분이 늘어나므로 **배울 게 있는 이슈에서만** 켠다. 마찰 관리는 대상 범위를 좁히는 대신 **개수 상한**으로 한다. 대상 K 는 `/wtflow:plan` 이 표에 `❓` 로 표시하고, `/wtflow:auto` 도 그 표시가 붙은 K 앞에서는 멈추고 묻는다. 무중단으로 돌리려면 `--no-ask` 를 준다(그러면 설계 선택과 예측만 순회 뒤로 모아 낸다). 계산이나 API 트리비아처럼 **정답이 하나인 것은 대상이 아니다** — 맞혀도 판단력이 늘지 않는다.
