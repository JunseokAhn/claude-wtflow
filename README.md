...
**note 는 쓰고 나면 건드리지 않는다.** 이슈 본문에는 "무엇/왜"만 남기고 페이로드·타입·에러코드 같은 상세는 note 로 뺀다. 그 note 를 `/wtflow:issue` 가 **이슈당 한 번** 쓰고, `plan` 은 읽기만 한다 — 갱신 의무가 없다. 계획 파일이 부담이었던 건 존재해서가 아니라 진행 내내 갱신해야 해서였고, 쓰는 자리를 한 번뿐인 지점으로 옮겨 그 비용을 없앴다. 대가는 note 가 낡을 수 있다는 것이고, 어긋나면 이슈 본문이 이긴다.
