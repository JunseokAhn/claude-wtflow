...
**설계 판단** — 메서드 분리·파라미터 설계·상수 여부·저장 위치·주석 범위·테스트 형태처럼 **내가 임의로 정하고 넘어갈 뻔한 선택**은 두 안을 나란히 내고 고르게 한다(K 당 1건). 거기에 "이 선택은 무엇이 바뀔 때 먼저 무너지나" 라는 반증 가능한 예측을 붙이고, 대가가 드러나는 K 에서 **예측과 실제를 숫자로 대조해 청구서를 낸다**. 고르고 끝내면 설문이지 훈련이 아니다 — 코드를 위임하면 코드가 느는 속도는 그대로인데 청구서를 받는 사람이 사라진다.
